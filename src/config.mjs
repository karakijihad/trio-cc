...
// One line, no backticks, 1-200 characters: the shape a path must have before
// it is written into a lens prompt or used as a promotion directory. Unicode
// line separators (U+0085, U+2028, U+2029) count as line breaks too, since a
// model reads them as one. Checked by code point, not by a regex, so this
// source never has to carry raw control characters. Shared by config
// validation, prompt.mjs and promote.mjs.
export function isSingleLinePath(s) {
  if (typeof s !== "string" || s.length < 1 || s.length > 200) return false;
  for (const ch of s) {
...
export function configErrors(cfg) {
  ...
  // It is interpolated into every lens brief (prompt.mjs), so a value that
  // could carry instructions — a newline, a backtick — is refused here.
  const promoteTo = at(cfg, "artifacts.promoteTo");
  if (
    typeof promoteTo !== "string" ||
    !isSingleLinePath(promoteTo)
  )
    errors.push(
      `artifacts.promoteTo must be a single-line path without backticks, got: ${JSON.stringify(promoteTo)}`,
    );
