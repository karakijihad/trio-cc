...
now, or silently bypasses a safety invariant), `major` (significant risk), `minor` (a worthwhile improvement, not
...
`continue` and `extend` refuse to advance past a pass with live findings that
have no verdict in its `verdicts.json`; `--unadjudicated` overrides that, and the pass is then marked
...
- `/trio:model [lens|consult] [model] [effort]` — interactive picker: pick a
  lens (or `consult`), pick its model from the live catalogue, pick an effort
  from the levels that model actually supports.
- `/trio:auditor`, `/trio:security`, `/trio:tester`, `/trio:simplifier`,
...
| `codex.lenses[]`                | five entries, all `on` | Each entry: `name`, `model`, `effort`, `on`. All five ship `model: null`/`medium`: `null` defers to the Codex CLI's own default, so no shipped slug can expire under you — at the cost of the run record only being able to say `codex default`. Pin a model with `/trio:model` or `/trio:lens`, or edit `.trio/config.json` directly; `trio models` lists what your CLI actually offers and your choice is validated against that catalogue. A pinned slug that has left the catalogue is flagged at run start, and Claude offers to pick a current one. One that is still listed but marked for retirement is only caught at session start: at session start, any lens or consult that is unpinned, pinned to a model gone from the catalogue, or pinned to one Codex marks for retirement is listed with its replacement (the retiring model's named upgrade, else your `~/.codex/config.toml` default) for Claude to offer; `trio models --apply` writes them. |
...
| `artifacts.archiveAfterDays`    | `7`                    | At session start, runs in `.trio/runs` untouched this many days move to `.trio/archive/<YYYY-Www>/` — moved, never deleted; the active run always stays. `null` turns it off. |
