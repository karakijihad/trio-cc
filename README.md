...
| `codex.lenses[]`                | five entries, all `on` | Each entry: `name`, `model`, `effort`, `on`. All five ship `model: null`/`medium`: `null` defers to the Codex CLI's own default, so no shipped slug can expire under you — at the cost of the run record only being able to say `codex default`. Pin a model with `/trio:model` or `/trio:lens`, or edit `.trio/config.json` directly; `trio models` lists what your CLI actually offers and your choice is validated against that catalogue. A pinned slug that is later retired is flagged at run start, and Claude offers to pick a current one. At session start, any lens or consult that is unpinned, pinned to a model gone from the catalogue, or pinned to one Codex marks for retirement is listed with its replacement (the retiring model's named upgrade, else your `~/.codex/config.toml` default) for Claude to offer; `trio models --apply` writes them. |
